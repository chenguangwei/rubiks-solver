...
- **Solver:** [`cubejs`](https://www.npmjs.com/package/cubejs) — a JS port of Kociemba's two-phase algorithm. Returns near-optimal solutions (typically 20-22 moves) in milliseconds after a one-time ~3-second pruning-table init. God's Number is 20, so there's a small gap to optimal — see the roadmap.
...
├── solver.ts          # cubejs wrapper, unsolvable-state detection
...
- **Solutions are typically 20-22 moves**, not always strictly optimal. Kociemba's two-phase algorithm trades a small move-count gap for very fast solve times; God's Number proves ≤20 moves is always achievable. See roadmap below for the upgrade path.
...
- [ ] **Tighter solutions** — exposed via a "tightest possible solution" toggle:
  - Run Kociemba longer (it iterates Phase 1 lengths and keeps the best total; we currently take the first solution it finds). Cheapest upgrade, ~1-2 moves saved.
  - Swap `cubejs` for a [min2phase](https://github.com/cs0x7f/min2phase) port (Chen Shuang) — heavier pruning tables and symmetry reduction, almost always returns 20 moves in milliseconds.
  - Korf-style optimal IDA* solver (max of three pruning tables: corners + two halves of edges) — guaranteed ≤20 moves but slower per cube.
