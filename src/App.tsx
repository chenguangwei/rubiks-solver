...
function repairColorCounts(input: string): string {
  ...
  const counts = countFaces(input)
  const missing = FACES.flatMap((face) =>
    Array.from({ length: Math.max(0, 9 - counts[face]) }, () => face),
  )
  for (let i = 0; i < chars.length && missing.length > 0; i++) {
...
function App() {
  ...
  const scanNeedsMoreFaces = false
  const canRequestSolve = canSolve && !scanNeedsMoreFaces && !noHintChallengeActive
  ...
  const validationMessage = !validation.ok
    ...
    : !reachable
      ...
      : scanNeedsMoreFaces
      ? 'Capture each face or import a complete net before solving.'
      : activeTab === 'scan'
  ...
  function applyRepairSuggestion() {
    setStateAndClearMoves(repairColorCounts(state))
  ...
  function startCasePractice(caseId: string) {
    setPracticeMessage(`Case practice started: ${caseId}`)
  ...
  return (
    <main className={`app-shell theme-${themePreference} ${reducedMotion ? 'reduce-motion' : ''} ${dragging ? 'dragging' : ''}`}>
      <input
        ref={fileInputRef}
        type="file"
        accept="image/*"
      ...
      <section className={`workspace workspace-${activeTab} ${showStepMode ? 'workspace-has-steps' : ''}`}>
        ...
        <aside className="capture-panel panel">
          <>
            ...
            <div className="setup-actions">
              ...
              <button onClick={() => cameraInputRef.current?.click()}>Use Camera</button>
        ...
        <aside className="inspector-stack">
          ...
          <section className={`validation-card ${validationOk ? 'valid' : 'invalid'}`}>
            ...
            {hasRepairSuggestion && (
              <button onClick={applyRepairSuggestion}>Apply Repair Suggestion</button>
            )}
          </section>
