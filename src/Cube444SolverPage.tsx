...
import { Cube4443D } from './Cube4443D'
...
import type { Cube444State } from './puzzles/444'
...
const PUZZLE_VALUE = msgKey('practice.short.444')
const MANUAL_MOVES = ['U', 'R', 'F', 'D', 'L', 'B', 'Uw', 'Rw', 'Fw', 'Dw', 'Lw', 'Bw'] as const

function statusLabel(status: Cube444Status, t: Translate): string {
  return t(`practice.status.${status}`)
}

function stateFingerprint(state: Cube444State): string {
  let hash = 0
...
export function Cube444SolverPage() {
  const { t } = useI18n()
  const puzzleName = t('practice.short.444')
  const displayName = t('practice.display.444')
  const [state, setState] = useState<Cube444State>(SOLVED_444_STATE)
  const [solutionHint, setSolutionHint] = useState<string[] | null>([])
  const [moves, setMoves] = useState<string[] | null>(null)
  const [solutionStates, setSolutionStates] = useState<Cube444State[]>([])
  ...
  const [status, setStatus] = useState<Cube444Status>('loading')
  ...
  const stickers = useMemo(() => stickersFor444State(displayState), [displayState])
  ...
  function setPlaybackStep(nextStep: number) {
    ...
    if (clamped === stepIndex + 1) {
      ...
      setPlaybackTurnMove(invert444Move(moves[clamped] ?? ''))
    } else {
      setPlaybackTurnMove(null)
    }
  ...
  async function handleRandomScramble() {
    ...
    try {
      const result = await cube444Adapter.randomStateWithScramble()
      setState(result.state)
      setSolutionHint(invert444Moves(result.scrambleMoves))
      setStatus('ready')
  ...
  function handleScannerComplete(facelets: string) {
    ...
    const validation = validate444State(facelets)
    if (!validation.ok) {
      setStatus('error')
  ...
  async function handleManualMove(move: string) {
    if (!canManualMove || !parse444Move(move)) return
    setBusy(true)
    clearSolution()
    setManualActive(true)
    ...
    try {
      const nextState = await cube444Adapter.applyMoves(state, move)
      const solved = await cube444Adapter.isSolved(nextState)
      setState(nextState)
      setSolutionHint((current) => (current ? [invert444Move(move), ...current] : null))
  ...
  async function handleSolve() {
    ...
    try {
      ...
      const states: Cube444State[] = [state]
      let cursor = state
      for (const move of solution) {
        cursor = await cube444Adapter.applyMoves(cursor, move)
        states.push(cursor)
      }
      const solvedState = states[states.length - 1] ?? state
      const verified = await cube444Adapter.isSolved(solvedState)
      setMoves(solution)
  ...
  return (
    <section className="workspace workspace-scan workspace-mini2x2 workspace-cube444" aria-label={displayName}>
      {showScanner && (
      ...
      <section className="stage-panel panel mini-cube-stage-panel">
        <div className="stage-toolbar">
          <div>
            <div className="panel-heading compact">
              ...
              <div>
                <h2>{t('practice.control.444.heading')}</h2>
                <p>{t('practice.control.444.description')}</p>
              </div>
            </div>
          </div>
        ...
        <div className="main-control-screen mini-cube-main-control">
          <div className="main-cube-frame mini-cube-main-frame" aria-label="4x4 main cube frame">
            <Cube4443D state={displayState} turnMove={playbackTurnMove} />
            ...
            <div className="corner-actions top-right">
              <button
                ...
                aria-label={t('practice.random.444')}
              >
                <span aria-hidden="true">↻</span>
            ...
            {canPlayback && (
              <div className="step-hud mini-cube-step-hud" aria-label="4x4 current step">
            ...
            {canPlayback ? (
              <div className="floating-playback-dock" aria-label="4x4 playback controls">
              ...
              <section className="solve-choice-dock mini-cube-solve-dock" aria-label="4x4 solve dock">
                <button
          ...
          {manualActive && !canPlayback && (
            <section className="main-manual-panel mini-cube-manual-panel" aria-label="4x4 manual controls">
              <div className="operation-title-row">
                <h2>{t('main.manualSolve')}</h2>
                <span>{t('practice.manualMoveCount', { count: manualMoves.length })}</span>
              </div>
              <div className="manual-controls cube-444-manual-controls">
          ...
          <section className="solve-status-strip">
            ...
            <div className="status-metrics" aria-label="4x4 state summary">
              <span>{statusLabel(status, t)}</span>
              ...
              <span>{t('practice.metric.stickers', { count: 96 })}</span>
            </div>
          </section>

          <div className="mini-cube-state-a11y">
            {stickers.map((face, index) => (
              <span key={`${index}-${face}`} aria-label={`4x4 sticker ${index + 1} ${face}`} />
            ))}
          </div>

          <section className="main-route-panel mini-cube-route" aria-label="4x4 solution route">
            ...
            {moves && moves.length > 0 ? (
              <div className="compact-moves" aria-label="4x4 move sequence">
